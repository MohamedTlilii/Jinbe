...
const sendRunSummary = async ({ nouvelles, reouvertures, demenagements, duration }) => {
  ...
  const total = nouvelles.length + reouvertures.length + demenagements.length;
  if (total === 0) return;
  ...
  <div style="font-family:sans-serif;background:#0f1117;color:#e8eaf0;padding:24px;border-radius:12px;max-width:700px">
    <h2 style="color:#6c63ff;margin-bottom:4px">◈ SecuLeads — Nouveau rapport</h2>
    ...
    <div style="display:flex;gap:16px;margin:20px 0">
      <div style="background:#1a1d27;padding:16px 24px;border-radius:10px;text-align:center">
        <div style="font-size:28px;font-weight:700;color:#22c55e">${nouvelles.length}</div>
        <div style="font-size:12px;color:#8b90a7">Nouvelles</div>
      </div>
      <div style="background:#1a1d27;padding:16px 24px;border-radius:10px;text-align:center">
        <div style="font-size:28px;font-weight:700;color:#f97316">${reouvertures.length}</div>
        <div style="font-size:12px;color:#8b90a7">Réouvertures</div>
      </div>
      <div style="background:#1a1d27;padding:16px 24px;border-radius:10px;text-align:center">
        <div style="font-size:28px;font-weight:700;color:#3b82f6">${demenagements.length}</div>
  ...
  await transporter.sendMail({
    from:    `"SecuLeads" <${EMAIL_USER}>`,
    to:      EMAIL_TO,
    subject: `SecuLeads — ${total} leads trouvés`,
    html,
  });
};
